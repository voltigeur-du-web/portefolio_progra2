...
bool add_to_dict(Dico_t dico, const char* key, long long int value);
...
long long int search_in_dict(Dico_t dico, char* key);
...
//POST: retourne un boleen : si il existe (key, value) in dico : dict' = dict \ {(key, value)} U {(key, value')}
    ...
    //false: si la clé n'existe pas dans le dictionnaire, pas de modification du dictionnaire
bool modify_in_dict(Dico_t dico, char* key, long long int value);
